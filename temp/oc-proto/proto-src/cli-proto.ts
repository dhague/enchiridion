...
/** PROTOTYPE — the in-process (plugin) entry. Captures stdout/stderr instead
 * of writing to the process streams, overrides commander's exit so neither
 * help() nor an error exit can terminate the host (OpenCode) process, and
 * returns the result as data. Guards: commander's exitOverride makes exit a
 * thrown CommanderError, not process.exit; help() is routed to the captured
 * streams. */
...
export async function run(argv: string[]): Promise<RunResult> {
  ...
  // stream writes for the duration of the run so ALL output is captured, then
  // restore — the host process (OpenCode) keeps its own streams untouched.
...
// a host (OpenCode plugin) can import it without hijacking the process.
