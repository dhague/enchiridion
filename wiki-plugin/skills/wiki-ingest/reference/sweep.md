...
1. **Run the scan.** `python "${CLAUDE_PLUGIN_ROOT}/scripts/ingest_scan.py" <folder-or-empty>` (or `--json` for machine-readable output) prints every file needing ingestion with reason:
...
2. **Print list, then ask.** Miscount — file missing from list, or one that shouldn't be there — easier to catch before anything written. Offer **all / none / choose**:
   ...
   - `choose` — per-file ask, each eligible file gets **yes / skip / never**:
     ...
     - `never` — `python "${CLAUDE_PLUGIN_ROOT}/scripts/ingest.py" --ignore <raw_rel>`. File never offered again unless policy edited. For file scanner offers only because ingested before [#34]'s `source/` stub-and-back-edge rule landed, add `--ignore-comment "ingested before back-pointers were mandatory"` so human reading later can tell cleanup from policy.
...
4. **On failure, move to next file.** Failed plan leaves already-written content uncommitted, consistent with `ingest.py`'s no-rollback stance; rerun safe once cause fixed. Don't abort sweep on single failure — surface in final summary, keep going.
