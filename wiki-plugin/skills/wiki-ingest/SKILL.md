...
6. **Write frontmatter** for each new or updated page via `python "${CLAUDE_PLUGIN_ROOT}/scripts/frontmatter.py" set <page> <key> <value>` (never hand-edit the YAML block; on an *updated* page, use `frontmatter.py merge` instead of `set` for `tags` and edge-list keys per step 3):
