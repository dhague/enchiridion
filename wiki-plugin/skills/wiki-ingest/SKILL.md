...
3. **Check for overlap.** Read `wiki/_index.md` and grep the vault for existing pages covering the same subject as any candidate chunk from step 2. For every existing page that surfaces, classify the relationship before deciding what to write:
   ...
   - **Substantive overlap, no conflict.** The candidate adds to, refines, or restates knowledge the existing page already carries, without contradicting it. **Update that page in place** rather than creating a duplicate: `Edit` its body and, via `frontmatter.py set`, whichever of `summary`/`tags`/`source_date`/`volatility` the new material actually changes. Record it as `updated`, not `created`, in the manifest and commit.
     - `frontmatter.py set` **overwrites** the key's whole value — it does not append. Before setting `tags` or any edge-list key (`refines`/`contradicts`/`example-of`/`source`/`related`/`supersedes`) on a page that already has one, first `frontmatter.py get <page> <key>` and union the existing entries with the new ones before writing the merged list back; setting only the new entries silently drops the page's existing tags/edges.
...
4. **Place each new page** using the fixed placement algorithm from `wiki-conventions`, first match wins: `source/` (stand-in for the raw artifact) → `synthesis/` (saved query result) → `entity/` (named, repeatedly-linked thing) → `concept/` (default). Filename is the lowercase kebab-slug of the title, no date prefix. (A page updated in place in step 3 keeps its existing location — this step only applies to genuinely new pages; the next bullet applies to new *and* updated pages alike.)
...
6. **Write frontmatter** for each new or updated page via `python scripts/frontmatter.py set <page> <key> <value>` (never hand-edit the YAML block; on an *updated* page, remember step 3's get-then-merge rule for `tags` and edge-list keys):
   ...
   - `supersedes` / typed-edge keys (`refines`/`contradicts`/`example-of`/`source`/`related`) — from step 4's judgment, each a list of quoted markdown links (`["[<title>](<relative/path.md>)"]`) via `frontmatter.py set <page> <key> <value> --json`. Set these on the new/updated page only — never touch a superseded page's own frontmatter.
...
9. **Commit.** Build a manifest (`title`, `action: "ingest"`, `created`/`updated` vault-relative paths, `superseded` as `[old, new]` pairs for every contradiction resolved this pass, `source_date`, `extra_paths: ["wiki/_index.md"]`) as a JSON file, then run `python scripts/commit.py --manifest <manifest.json>`.
