...
**Migrated to Go:** the script layer is being replaced subcommand by subcommand with one static binary needing no Python ([ADR-0011](../../../docs/adr/0011-go-rewrite-scope-sequencing-toolchain.md)). Migrated entries call `<plugin-root>/bin/enchiridion <subcommand>` instead of `python <plugin-root>/scripts/<name>.py` — same flags, same output. That wrapper lazy-fetches the platform binary on first use; nothing to install. Table below is the authority on which is which — no fallback, so a row says one or the other, never both.
...
| `commit.py` | Writing one structured git commit per ingestion/edit manifest: stages paths, gates on chain-of-evidence, returns SHA. Only for a hand-built manifest — `enchiridion ingest` commits its own plan. | `commit.py --manifest <path>`. |
| `chain_of_evidence.py` | (library only) Enforcing page → source stub → raw file chain. No CLI on either side; `enchiridion ingest` runs it as pre-flight *and* commit-time gate. | No CLI. `check(staged, raw)` → list of errors. |
